...
void and_result(player_t *player, int rd_val[2][3])
{
    int val1 = 0;
    int val2 = 0;
...
{
    int decal_ind[2] = {2, player->program_counting + 2};
    int param[4] = {
    ...
    for (int i = 0; i < 3; i++) {
        get_value_and(rd_vl[0][i], &rd_vl[1][i], data->memory, &decal_ind[1]);
        decal_ind[0] += rd_vl[0][i];
    }
    player->cycle_wait = op_tab[5].nbr_cycles;
    and_result(player, rd_vl);
    return (decal_ind[0]);
}