...
void get_value(int index, int to_read[2], unsigned char *integer, data_t *data)
{
...
void get_memory(player_t *player, unsigned char *integer, data_t *data)
{
    int pos = player->program_counting + *(int *)integer % IDX_MOD;
...
void get_info_param(unsigned char param, int to_read[2], int *decal)
{
    if (param == 2)
        to_read[0] = DIR_SIZE;
    else if (param == 3)
